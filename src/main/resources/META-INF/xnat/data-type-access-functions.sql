CREATE OR REPLACE VIEW public.data_type_views_element_access AS
  SELECT coalesce(g.id, 'user:' || u.login) AS entity, a.element_name AS element_name, m.field_value, m.field, m.active_element, m.read_element, m.edit_element, m.create_element, m.delete_element, m.comparison_type, s.method, a.xdat_element_access_id AS xdat_element_access_id, s.xdat_field_mapping_set_id AS xdat_field_mapping_set_id, m.xdat_field_mapping_id AS xdat_field_mapping_id
  FROM
...
BEGIN
  ...
  THEN
    ...
    UNION
    SELECT field, a.id, 0, 1, 0, 0, 1, 'equals', xdat_field_mapping_set_id
    FROM
      data_type_views_missing_mapping_elements e,
      project_access a
    WHERE
