...
public class DefaultPrearchiveOperationHandlerResolver implements PrearchiveOperationHandlerResolver {
    @Autowired
    public DefaultPrearchiveOperationHandlerResolver(final NrgEventServiceI eventService, final XnatUserProvider receivedFileUserProvider) {
        ...
        for (final Class<? extends PrearchiveOperationHandler> handler : Iterables.filter(reflections.getSubTypesOf(PrearchiveOperationHandler.class), ReflectionUtils.withAnnotation(Handles.class))) {
            try {
                ...
                _handlers.put(handler.getAnnotation(Handles.class).value(), handler.getConstructor(PrearchiveOperationRequest.class, NrgEventServiceI.class, XnatUserProvider.class));
    ...
    public PrearchiveOperationHandler getHandler(final PrearchiveOperationRequest request) {
        ...
        try {
            return constructor.newInstance(request, _eventService, _userProvider);
