...
public class XnatTaskServiceImpl implements XnatTaskService {
     ...
     */
    @Autowired
    public XnatTaskServiceImpl(final XnatNode xnatNode, final SiteConfigPreferences preferences, final XnatTaskInfoService taskInfoService, final LoggingService loggingService, final XnatTaskExecutionResolverI[] executionResolvers) {
        _xnatNode = xnatNode;
    ...
    public boolean shouldRunTask(final Class<?> clazz) {
        ...
        log.warn("The shouldRunTask() method for the task \"{}\" was not able to match any configuration. The task will be run.", clazz.getName());
        return true;
    }

    @Override
    public void start(final XnatTaskI task) {
        _loggingService.start(task);
    }

    @Override
    public void update(final XnatTaskI task, final String message, final Object... parameters) {
        _loggingService.update(task, message, parameters);
    }

    @Override
    public void finish(final XnatTaskI task) {
        _loggingService.finish(task);
    }
